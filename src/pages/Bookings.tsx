...
const Bookings: React.FC = () => {
  ...
  const [bookingForPayment, setBookingForPayment] = useState<number | null>(
    null
  );
  ...
  const applyFilters = () => {
    ...
    if (startDate || endDate) {
      ...
      filtered = filtered.filter(booking => {
        if (booking.rawData) {
          const checkInDate = new Date(booking.rawData.check_in);
    ...
    if (paymentStatusFilter) {
      ...
      switch (paymentStatusFilter) {
        case 'success': statusToFilter = 'Paid'; break;
        case 'partial': statusToFilter = 'Partial'; break;
        case 'failed': statusToFilter = 'Unpaid'; break;
        case '': statusToFilter = 'Pending'; break;
      }
      
      if (statusToFilter) {
        filtered = filtered.filter(booking => booking.paymentStatus === statusToFilter);
      }
    }
    ...
    if (bookingStatusFilter) {
      ...
      switch (bookingStatusFilter) {
        case 'confirmed': statusToFilter = 'Confirmed'; break;
        case 'pending': statusToFilter = 'Pending'; break;
        case 'cancelled': statusToFilter = 'Cancelled'; break;
      }
      
      if (statusToFilter) {
        filtered = filtered.filter(booking => booking.bookingStatus === statusToFilter);
      }
    }
    ...
    // Set initial page of filtered results
  ...
  }, [searchTerm, startDate, endDate, paymentStatusFilter, bookingStatusFilter]);
  ...
  }, [pagination.page, pagination.limit, filteredBookings]);
  ...
  const handleResetFilters = () => {
    ...
    setBookings(allBookings.slice(0, pagination.limit));
  };
  ...
  const getModalCompatibleBooking = (booking: Booking) => {
    return {
      ...booking,
      paymentStatus:
        booking.paymentStatus === "Pending" ? "Unpaid" : booking.paymentStatus,
    };
  };
    ...
    <div className="flex flex-col h-screen">
                  ...
                  {bookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="flex flex-col">
                          <span>
                            Adult guests: <span>{booking.adults}</span>
                          </span>
                          <span>
                            Chilldren guests: <span>{booking.children}</span>
                          </span>
                          <span>
                            Veg Count: <span>{booking.veg}</span>
                          </span>
                          <span>
                            Non-Veg Count: <span>{booking.nonVeg}</span>
                          </span>
                          <span>
                            Jain Count: <span>{booking.jainCount}</span>
                          </span>
                        </div>
                      </td>
      ...
      {selectedBooking !== null &&
        bookings.find((b) => b.id === selectedBooking) && (
          <BookingDetailsModal
            booking={getModalCompatibleBooking(
              bookings.find((b) => b.id === selectedBooking)!
            )}
